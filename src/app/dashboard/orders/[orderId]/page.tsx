...
function StatusIcon({ status }: { status: string }) {
  ...
  if (status === "ready") {
    return (
      <svg className="h-8 w-8" viewBox="0 0 24 24" fill="none">
        <path d="M20 7 12 3 4 7m16 0-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    );
  }
  ...
  if (status === "processing") {
    return (
      <svg className="h-8 w-8" viewBox="0 0 24 24" fill="none">
        <path d="M12 3v4m0 10v4M4.93 4.93l2.83 2.83m8.48 8.48 2.83 2.83M3 12h4m10 0h4M4.93 19.07l2.83-2.83m8.48-8.48 2.83-2.83" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" />
      </svg>
    );
  }
  ...
  return (
    <svg className="h-8 w-8" viewBox="0 0 24 24" fill="none">
      <path d="M12 8v4l2.5 2M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
export default async function DashboardOrderDetailPage({ params }: Props) {
  ...
  return (
    <DashboardFrame
      ...
      description="Pantau item layanan, pembayaran, dan aksi outlet dari satu halaman operasional yang lebih lengkap."
    >
      <section className="mb-8 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <Link href="/dashboard/orders" className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-brand/40 transition hover:text-accent">
      ...
      <section className="mb-8 overflow-hidden rounded-[2.2rem] bg-surface-soft p-6 md:p-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-5">
            <div className={`flex h-20 w-20 items-center justify-center rounded-[1.6rem] shadow-[0_18px_38px_rgba(25,28,30,0.08)] ${statusClass}`}>
              <StatusIcon status={order.status} />
            </div>
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-brand/30">
                Fase Saat Ini
              </p>
            ...
            <div className="mb-8 flex items-center gap-4">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-surface-soft text-accent">
                <svg className="h-6 w-6" viewBox="0 0 24 24" fill="none">
                  <path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2M9 5a2 2 0 0 0 2 2h2a2 2 0 0 0 2-2M9 5a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
              <div>
                ...
                {order.items.map((item) => (
                  <article key={item.id} className="grid gap-4 px-5 py-5 md:grid-cols-[minmax(0,1fr)_110px_140px_160px] md:items-center">
                    ...
                    <div className="flex items-center justify-between md:block md:text-center">
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-brand/35 md:hidden">
                        Jumlah
                      </span>
                      <p className="text-sm font-bold text-brand">
                        {item.quantity} <span className="text-[10px] font-black uppercase tracking-[0.18em] text-brand/40">{item.unit ?? "unit"}</span>
                      </p>
                    </div>
                    ...
                    <div className="flex items-center justify-between md:block md:text-right">
                      ...
                      <p className="font-[var(--font-display-sans)] text-lg font-extrabold text-brand">
              ...
              <div className="border-t border-line/35 bg-surface-soft/65 px-5 py-5">
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between gap-4 border-t border-line/35 pt-4">
                    <span className="text-[10px] font-black uppercase tracking-[0.25em] text-brand/40">
                      Total Tagihan
                    </span>
          ...
          <section className="rounded-[2rem] border border-line/35 bg-white p-6 shadow-[0_12px_28px_rgba(25,28,30,0.04)] md:p-8">
            ...
            {order.notes ? (
              <div className="mt-8 border-t border-line/35 pt-6">
                ...
                <p className="mt-3 text-sm font-medium italic leading-7 text-brand/75">
        ...
        <div className="space-y-8">
          ...
          <section className="rounded-[2rem] border border-line/35 bg-white p-6 shadow-[0_12px_28px_rgba(25,28,30,0.04)]">
            <div className="mb-6 flex items-center gap-4">
              ...
              <h3 className="text-lg font-[var(--font-display-sans)] font-extrabold text-brand">
                Pembayaran
              </h3>
            </div>

            <div className={`rounded-[1.5rem] p-5 ${paymentClass}`}>
              <p className="text-[10px] font-black uppercase tracking-[0.22em] text-brand/40">
                Status Pembayaran
              </p>
              <p className="mt-2 text-2xl font-[var(--font-display-sans)] font-extrabold">
                {order.payment_status_label}
              </p>
              <p className="mt-3 text-[10px] font-black uppercase tracking-[0.18em] text-brand/45">
                Metode: {order.payment_method_label}
              </p>
            </div>
            ...
            {order.payment_proof_url ? (
              <div className="mt-6 space-y-4 rounded-[1.5rem] border border-line/35 bg-white p-4">
                ...
                {order.payment_notes ? (
                  <p className="text-sm leading-7 text-muted">{order.payment_notes}</p>
                ) : null}
            ...
            {!order.payment_proof_url && order.payment_status !== "paid" && order.outlet?.qris_image_url ? (
              <div className="mt-6 rounded-[1.5rem] border border-line/35 bg-white p-4">
                ...
                <p className="mt-4 text-sm leading-7 text-muted">
                  {order.outlet.qris_notes || "Gunakan QRIS outlet ini bila pelanggan ingin membayar non-tunai secara manual."}
                </p>
              </div>
            ) : null}
          </section>
          ...
          <section className="section-dark hero-card p-5 sm:p-6">
            <p className="section-label-dark">Catatan operasional</p>
            <div className="info-list mt-5 text-sm">
              ...
              <div className="kpi-pill-dark">
                ...
                <p className="mt-1 text-white/72">{order.outlet?.name ?? "Outlet"}</p>
              </div>
