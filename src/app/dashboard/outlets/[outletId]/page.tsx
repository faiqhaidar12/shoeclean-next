...
import { getOutlet } from "@/lib/auth";
...
export default async function DashboardOutletDetailPage({ params }: Props) {
    ...
    if (error instanceof ApiError && error.status === 403) {
      return (
        <DashboardFrame
          ...
          title="Outlet ini tidak bisa diakses."
          description="Akses edit outlet dibatasi ke owner outlet tersebut atau admin yang memang ditempatkan di outlet itu."
        >
          <section className="rounded-[2rem] bg-white p-6 shadow-[0_12px_28px_rgba(25,28,30,0.04)]">
            <p className="text-sm text-muted">
              Jika akun ini memang perlu akses lebih luas, kita bisa evaluasi ulang policy outlet di
              langkah berikutnya.
            </p>
          </section>
        </DashboardFrame>
  ...
  return (
    <DashboardFrame
      ...
      description="Perbarui identitas outlet, status operasional, biaya layanan, dan QRIS agar storefront selalu sinkron."
    >
